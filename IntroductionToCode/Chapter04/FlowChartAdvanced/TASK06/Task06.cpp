...
int main()
{
	...
	printf("\nPhuong trinh: %lf*x^4 + %lf*x^2 + %lf = 0\n", a, b, c);
...
void inKetQua(double x1, double x2, unsigned int soNghiem)
{
	...
	{
		if (x1 == 0) printf("x = 0");
		else if (x1 < 0) printf("Phuong trinh vo nghiem");
		else
		{
			printf("x1 = %lf\n", -sqrt(x1));
	...
	else if (soNghiem == 2)
	{
		...
		else if (x2 > 0)
		{
			...
			printf("x%u = %lf\n", demNghiem, -sqrt(x2));
			demNghiem++;
			printf("x%u = %lf", demNghiem, sqrt(x2));
		}

		if (demNghiem == 0) printf("Phuong trinh vo nghiem");
	}
