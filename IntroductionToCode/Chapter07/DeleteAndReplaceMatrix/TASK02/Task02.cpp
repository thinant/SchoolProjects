...
void xoaCotTrenMaTran(MANG2D_THUC maTran, unsigned int soDong, unsigned int& soCot, unsigned int cotCanXoa);
void xuatMangThuc(double mangThuc[], unsigned int soPhanTu);

int main()
{
	MANG2D_THUC maTran{ 0 };
	unsigned int soDong = 0, soCot = 0, cotCanXoa = 0;
	...
	xoaCotTrenMaTran(maTran, soDong, soCot, cotCanXoa);

	printf("Ma tran sau khi xoa cot %u la:\n", cotCanXoa);
	xuatMaTranThuc(maTran, soDong, soCot);
...
{
	for (unsigned int i = 0; i < soDong; i++)
		for (unsigned int j = cotCanXoa; j < soCot - 1; j++)
