...
void xoayMaTran90(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot);
...
int main()
{
	...
	xoayMaTran90(maTran, soDong, soCot);
	printf("Ma tran sau khi xoay 90*:\n");
	xuatMaTranThuc(maTran, soCot, soDong);
...
void xoayMaTran90(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot)
{
	MANG2D_THUC maTranXoay{ 0 };
	for (unsigned int i = 0; i < soDong; i++)
		for (unsigned int j = 0; j < soCot; j++)
			maTranXoay[soCot - 1 - j][i] = maTran[i][j];

	for (unsigned int i = 0; i < soCot; i++)
		for (unsigned int j = 0; j < soDong; j++)
			maTran[i][j] = maTranXoay[i][j];
