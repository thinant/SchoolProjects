...
void taoMaTranTrungBinhCong(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot, MANG2D_THUC maTranTrungBinhCong);
double tinhTrungBinhCongMaTranVuong(MANG2D_THUC maTran, int soDong, int soCot, int tam_i, int tam_j);

int main()
{
	MANG2D_THUC maTran, maTranTrungBinhCong;
	...
	taoMaTranTrungBinhCong(maTran, soDong, soCot, maTranTrungBinhCong);

	printf("Ma tran trung binh cong tao thanh tu ma tran tren la:\n");
	xuatMaTranThuc(maTranTrungBinhCong, soDong, soCot);
...
void taoMaTranTrungBinhCong(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot, MANG2D_THUC maTranTrungBinhCong)
{
	for (unsigned int i = 0; i < soDong; i++)
		for (unsigned int j = 0; j < soCot; j++)
			maTranTrungBinhCong[i][j] = tinhTrungBinhCongMaTranVuong(maTran, soDong, soCot, i, j);
}
...
double tinhTrungBinhCongMaTranVuong(MANG2D_THUC maTran, int soDong, int soCot, int tam_i, int tam_j)
{
	...
	for (int i = tam_i - 1; i < soDong && i <= tam_i + 1; i++)
	{
		...
		{
			if (j < 0) continue;
			tongMaTran += maTran[i][j];
			soPhanTuMaTran++;
		}
	}

	return tongMaTran / soPhanTuMaTran;
}