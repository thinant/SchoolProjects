...
void taoMaTranLatNguoc(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot, MANG2D_THUC maTranLat);

int main()
{
	MANG2D_THUC maTran, maTranLat;
	...
	taoMaTranLatNguoc(maTran, soDong, soCot, maTranLat);

	printf("Ma tran lat tao thanh tu ma tran tren la:\n");
	xuatMaTranThuc(maTranLat, soDong, soCot);
...
void taoMaTranLatNguoc(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot, MANG2D_THUC maTranLat)
{
	for (unsigned int i = 0; i < soDong; i++)
		for (unsigned int j = 0; j < soCot; j++)
			maTranLat[i][j] = maTran[i][soCot - 1 - j];
}