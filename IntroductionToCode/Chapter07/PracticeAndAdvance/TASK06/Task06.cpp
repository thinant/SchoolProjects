...
void xoayPhaiMaTran(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot);
...
int main()
{
	...
	xoayPhaiMaTran(maTran, soDong, soCot);
	printf("Ma tran sau khi xoay phai mot cot:\n");
	xuatMaTranThuc(maTran, soDong, soCot);
...
void xoayPhaiMaTran(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot)
{
	for (unsigned int i = 0; i < soDong; i++)
	{
		double phanTuCuoiHang = maTran[i][soCot - 1];
		for (int j = soCot - 1; j >= 0; j--)
			maTran[i][j] = maTran[i][j - 1];

		maTran[i][0] = phanTuCuoiHang;
	}
}