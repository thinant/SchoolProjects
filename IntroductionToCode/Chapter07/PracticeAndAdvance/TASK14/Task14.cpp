...
void timCucTriMaTran(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot, int maTranCucTri[TOI_DA][TOI_DA], unsigned int& soCucTri);
int main()
{
	MANG2D_THUC maTran;
	...
	timCucTriMaTran(maTran, soDong, soCot, maTranCucTri, soCucTri);

	printf("Co %d cuc tri trong ma tran:\n", soCucTri);
	
	for (unsigned int i = 0; i < soDong; i++)
		for (unsigned int j = 0; j < soCot; j++)
			if (maTranCucTri[i][j] == 2)
				printf("Phan tu [%d][%d]: %lf\n", i, j, maTran[i][j]);
...
void timCucTriMaTran(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot, int maTranCucTri[TOI_DA][TOI_DA], unsigned int& soCucTri) { 
	...
	for (unsigned int i = 0; i < soDong; i++) {
		double soLonNhat = maTran[i][0], soBeNhat = maTran[i][0];
		for (unsigned int j = 0; j < soCot; j++) {
			if (maTran[i][j] > soLonNhat)
				soLonNhat = maTran[i][j];
			if (maTran[i][j] < soBeNhat)
				soBeNhat = maTran[i][j];
		}
	...
	for (unsigned int i = 0; i < soCot; i++) {
		double soLonNhat = maTran[0][i], soBeNhat = maTran[0][i];
		for (unsigned int j = 0; j < soDong; j++) {
			if (maTran[j][i] > soLonNhat)
				soLonNhat = maTran[j][i];
			if (maTran[j][i] < soBeNhat)
				soBeNhat = maTran[j][i];
		}

		for (unsigned int j = 0; j < soDong; j++) {
			if (maTran[j][i] == soLonNhat && maTranCucTri[j][i] == -1) {
				maTranCucTri[j][i] = 2;
				soCucTri++;
			}
			if (maTran[j][i] == soBeNhat && maTranCucTri[j][i] == 1) {
				maTranCucTri[j][i] = 2;
				soCucTri++;
			}
		}
