...
void sapXepMaTranTheoDong(MATRAN_THUC maTran, unsigned int soDong, unsigned int soCot);
void sapXepDong(MATRAN_THUC maTran, unsigned int viTriDong, unsigned int soPhanTuTrenDong);
...
int main()
{
	...
	sapXepMaTranTheoDong(maTran, soDong, soCot);
...
{
	for (unsigned int i = 0; i < soDong; i++)
		sapXepDong(maTran, i, soCot);
}

void sapXepDong(MATRAN_THUC maTran, unsigned int viTriDong, unsigned int soPhanTuTrenDong)
{
	for (unsigned int i = 0; i < soPhanTuTrenDong - 1; i++)
		for (unsigned int j = i + 1; j < soPhanTuTrenDong; j++)
			if (maTran[viTriDong][i] > maTran[viTriDong][j])
				hoanVi(maTran[viTriDong][i], maTran[viTriDong][j]);
}
