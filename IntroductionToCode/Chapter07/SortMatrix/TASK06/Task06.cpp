...
void duaMangVaoMaTran(MATRAN_THUC maTran, unsigned int soDong, unsigned int soCot, double mang[])
{
	bool laChieuThuan = false;
	int dongThapNhat = 0, dongCaoNhat = 0, cotGanNhat = 0, cotXaNhat = 0, a = 1;
	int dong = dongCaoNhat, cot = cotGanNhat;
	...
	for (unsigned int i = 0; i < soDong * soCot; i++)
	{
		...
		{
			laChieuThuan = !laChieuThuan;

			if (cotXaNhat >= soCot - 1)
				dongThapNhat++;
			else
				cotXaNhat++;

			if (dongCaoNhat >= soDong - 1)
				cotGanNhat++;
			else
				dongCaoNhat++;
