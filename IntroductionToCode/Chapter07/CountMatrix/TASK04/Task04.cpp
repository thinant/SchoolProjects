...
void demPhanTuNhoHonTrenMaTran(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot, unsigned int& soPhanTuNhoHon, double phanTu);

int main()
{
	MANG2D_THUC maTran{ 0 };
	unsigned int soDong = 0, soCot = 0, soPhanTuNhoHon = 0;
	double phanTuNhoHon = 0;
	char giaTri[TOI_DA];

	nhapMaTranThuc(maTran, soDong, soCot);

	printf("Phan tu can xet la: ");
	scanf_s("%s", &giaTri, 20);
	...
	phanTuNhoHon = chuyenChuoiThanhSoThuc(giaTri);

	printf("Ma tran da nhap la:\n");
	xuatMaTranThuc(maTran, soDong, soCot);

	demPhanTuNhoHonTrenMaTran(maTran, soDong, soCot, soPhanTuNhoHon, phanTuNhoHon);

	printf("\nMang co %d phan tu nho hon %lf.", soPhanTuNhoHon, phanTuNhoHon);
