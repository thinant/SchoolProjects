...
unsigned int demPhanTuTrenMaTran(MANG2D_THUC maTran, unsigned int soDong, unsigned int soCot, double phanTu);
void xuatMangThuc(double mangThuc[], unsigned int soPhanTu);

int main()
{
	MANG2D_THUC maTran{ 0 };
	unsigned int soDong = 0, soCot = 0, soPhanTu = 0;
	double phanTu = 0;
	char giaTri[TOI_DA];

	nhapMaTranThuc(maTran, soDong, soCot);

	printf("Phan tu can dem la: ");
	scanf_s("%s", &giaTri, 20);
	...
	phanTu = chuyenChuoiThanhSoThuc(giaTri);

	printf("Ma tran da nhap la:\n");
	xuatMaTranThuc(maTran, soDong, soCot);

	soPhanTu = demPhanTuTrenMaTran(maTran, soDong, soCot, phanTu);

	printf("\nMang co %d phan tu %lf.", soPhanTu, phanTu);
