...
void timCacPhanTuNhoNhatDuongCheoPhu(MANG2D_THUC maTran, unsigned int soDong, double mangNhoNhat[]);
void xuatMangThuc(double mang[], unsigned int soPhanTu);

int main()
{
	MANG2D_THUC maTran{ 0 };
	unsigned int soDong = 0;
	double mangNhoNhatCacDuongCheoPhu[TOI_DA]{ 0 };
	...
	timCacPhanTuNhoNhatDuongCheoPhu(maTran, soDong, mangNhoNhatCacDuongCheoPhu);

	printf("Cac phan tu nho nhat cua tung duong cheo phu la:");
	xuatMangThuc(mangNhoNhatCacDuongCheoPhu, 2 * soDong - 1);
...
void timCacPhanTuNhoNhatDuongCheoPhu(MANG2D_THUC maTran, unsigned int soDong, double mangNhoNhat[])
{
	for (unsigned int n = 0; n < soDong; n++)
	{
		double phanTuNhoNhat = 0;
		for (int i = 0, j = n; i <= n && j >= 0; i++, j--)
			if (i == 0 || phanTuNhoNhat > maTran[i][j])
				phanTuNhoNhat = maTran[i][j];
		mangNhoNhat[n] = phanTuNhoNhat;
	}

	for (unsigned int n = soDong; n <= 2 * (soDong - 1); n++)
	{
		double phanTuNhoNhat = 0;
		for (int  j = soDong - 1, i = n - j; i < soDong && j >= 0; i++, j--)
			if (j == soDong - 1 || phanTuNhoNhat > maTran[i][j])
				phanTuNhoNhat = maTran[i][j];
		mangNhoNhat[n] = phanTuNhoNhat;
	}
}
