...
void xuatPhanTuNguyenToDuongCheoPhu(MANG2D_NGUYEN maTran, unsigned int soDong);
...
int main()
{
	...
	printf("Cac phan tu nguyen to nam tren duong cheo phu la: ");
	xuatPhanTuNguyenToDuongCheoPhu(maTran, soDong);
...
{
	for (unsigned int i = 0; i < soDong; i++)
		if (kiemTraSoNguyenTo(maTran[i][soDong - 1 - i]))
			printf("\t%d", maTran[i][soDong - 1 - i]);
}
