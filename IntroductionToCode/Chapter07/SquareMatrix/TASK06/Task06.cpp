...
double timPhantuLonNhatDuongCheoChinh(MANG2D_THUC maTran, unsigned int soDong);
...
int main()
{
	...
	phanTuLonNhat = timPhantuLonNhatDuongCheoChinh(maTran, soDong);

	printf("Phan tu lon nhat tren duong cheo chinh cua ma tran la: %lf", phanTuLonNhat);
...
{	
	double phanTuLonNhat = 0;
	for (unsigned int i = 0; i < soDong; i++)
		if (i == 0 || phanTuLonNhat < maTran[i][i])
			phanTuLonNhat = maTran[i][i];
